<template lang="html">
  <div class="zhuanti250">
    ...
    <div class="bar" v-if="type==1">
      <div @click="changemain(index)" :class="nowpage==index?'active':''" v-for="(x,index) in list">{{x}}</div>
    </div>
        ...
        <li v-for="(x,index) in dat.rank.length" @click="push(id[index])">
          ...
          <div class="ma-bot">
            ...
            <div class="tick">
              ...
              <p>演员：<span v-for="(l,index3) in dat.sub[index].casts">{{index3 | actname(l.name,dat.sub[index].casts.length)}} </span>
              </p>
            </div>
          </div>
        </li>
